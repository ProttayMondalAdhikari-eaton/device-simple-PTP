...
/** The structure to hold an error code and optional message. */
typedef struct devsdk_error
{
  /** The error code, from errorlist.h. Is 0 for no error / success case. */
  uint32_t code;
  /** The optional reason for the error. Contents are not expected to be freed. */
  const char *reason;
} devsdk_error;
