...
/**
 * @brief Optional callback function called when any parameter of the Driver (or AppCustom) configuration is updated (or initialized).
 * Generally this is done via configuration from consul.
 * @param impl The context data passed in when the service was created.
 * @param config A string map containing the new Driver or AppCustom configuration.
 */
...
#ifdef USE_EATON_SDK_ENHANCEMENT

/**
 * @brief The template for a callback function to be called when an individual configuration parameter is updated via REST PUT.
 * Eg, via PUT /api/v2/config/params/{parentname}/{childparamname}
 * Callback of type devsdk_config_change_event
 * 
 * @param impl The application context given when we registered this callback.
 * @param keyChanged Name of the changed configuration parameter
 * @param keyValue The new (updated) value of the parameter
 */
typedef void (*devsdk_config_change_event) (void *impl, const char *keyChanged, const char *keyValue);


/**
 * @brief Callback issued for handling any type of event notification
 * Currently handled for MQTT later can be generalized for passing any type of event back to app
 *
 * @param impl : The context data passed in when the service was created.
 * @param topic_name : subscription topic (in case of MQTT)
 * @param msg_payload_len : message payload length
 * @param msg_payload : message payload content
 * @return void
 */
typedef void (*devsdk_event_notification) (void *impl, const char* topic_name, const int msg_payload_len, const char* msg_payload);

#endif	// USE_EATON_SDK_ENHANCEMENT
...
/**
 * @brief Set the optional reconfiguration callback function(s)
 * @param cb The service's structure to hold all callbacks
 * @param reconf Optional callback function called when any parameter of the Driver (or AppCustom) configuration is updated (or initialized).
 * @param config_changed Optional callback function called when an individual configuration parameter is updated via REST PUT.
 */

#ifdef USE_EATON_SDK_ENHANCEMENT
void devsdk_callbacks_set_reconfiguration (devsdk_callbacks *cb, devsdk_reconfigure reconf, devsdk_config_change_event config_changed);
#else
...
#ifdef USE_EATON_SDK_ENHANCEMENT
/**
 * @brief Register event notification function for apps. Generic for all type of event notification back to app
 * 
 * @param cb : Pointer to the devsdk callback
 * @param devsdk_event_notification : callback function registered of type devsdk_event_notification
 * @return void
 */
void devsdk_callbacks_set_event_notification (devsdk_callbacks *cb, devsdk_event_notification event_notification);
#endif
...
/**
 * @brief This methods allows custom events to be publsihed.
 * @param svc The device service.
 * @param topic The topic name to post custom events
 * @param data Event data.
 * @param json_type Flag to indicate the type of the data. [If true = JSON type else false = CBOR(Not supported for now)]
 */

void post_custom_events( devsdk_service_t *svc, const char* topic, const char* data, bool json_type );
...
/**
 * @brief Set the administrative state of a device
 * @param svc The device service.
 * @param devname The device name.
 * @param unlocked true if the device is in an unlocked adminState
 * @param err Nonzero reason codes will be set here in the event of errors.
 */

void devsdk_set_device_adminstate (devsdk_service_t *svc, char *devname, bool unlocked, devsdk_error *err);
...
// Define the basic topic prefixes for subscriptions
#define EVENTS_TOPIC_PREFIX	"edgex/events/"
#define DEVICE_EVENTS_TOPIC_PREFIX	"edgex/events/device/"

/**
 * @brief Subscribe to the topic either explicitly, or implicitly (by passing subscription_topic as NULL). 
 *        Currently it is only subscribing for MQTT Message Bus Event/s
 * @param svc The device service.
 * @param subscription_topic topic name for the subscription.
 *    If this is null, then it will subscribe to edgex/events/device/<profile-name>/<device-name>/#
 *    for the current device (device SDK builds only).
 *    Otherwise, provide a full subscription path, perhaps starting with EVENTS_TOPIC_PREFIX
 * @return True on success, else False if it failed to subscribe
 */
bool devsdk_subscribe_eventbus(void* pdevsdk_svc, const char* subscription_topic);

/**
 * @brief Unsubscribe from the given topic either explicitly, or implicitly (by passing unsub_topic as NULL). 
 *        Currently it only works for unsubscribing from MQTT Message bus Event/s
 * @param svc device service ptr
 * @param unsub_topic topic name to un-subscribe from
 *    If this is null, then it will unsubscribe from edgex/events/device/<profile-name>/<device-name>/#
 *    for the current device (device SDK builds only).
 *    Otherwise, provide a full unsubscription path, perhaps starting with EVENTS_TOPIC_PREFIX
 * @return True on success, else False if it failed to unsubscribe
 */
bool devsdk_unsubscribe_eventbus(void* pdevsdk_svc, const char* unsub_topic);

/*
 * @brief Get string value corresponding to key from a map
 * The function to get the string value from the driverconf map
 * @param svc devsdk_service_t ptr
 * @param paramsName input param name
 * @return const char*
 */
const char* edgex_device_get_config_params(devsdk_service_t *svc, const char *params_name);

/*
 * @brief  Wrapper function to upload SDK Driver's config map to Consul
 * During startup, if service's configuration unavailable, 
 * then only SDK uploads config to Consul.
 * so,this function helps to update the data to Consul during runtime as well.
 * 
 * @param svc devsdk_service_t ptr.
 * @param configmap parameter to be uploaded.
 * @param e Updated SDK error code and reason in case of any error.
 * @return void.
 */
void devsdk_registry_put (devsdk_service_t *svc, iot_data_t *configmap, devsdk_error *e);

/*
 * @brief  Wrapper function to get configuration data from SDK Driver's conf.
 * 
 * @param svc devsdk_service_t ptr.
 * @param key Corresponding key, whose value to be GET.
 * @return Value of requested key from configmap.
 */
const char* devsdk_get_config_data (devsdk_service_t *svc, const char *key);

/*
 * @brief  Wrapper function to get callback if any data alters SDK Driver's conf.
 *
 * @param svc devsdk_service_t ptr.
 * @param key Corresponding key, whose value is changed.
 * @return val updated value.
 */
void devsdk_config_changed( devsdk_service_t *svc, const char *key, const char *val );

#ifdef APPLICATION_SVC_SDK
/**
 * @brief Post readings to the core-data/message bus via App-SDK
 * This method allows readings to be generated other than in response to a device GET invocation.
 * @param svc The device service ptr.
 * @param device_name The name of the device that the readings have come from.
 * @param resource_name Name of the resource or command which defines the Event.
 * @param values An array of readings. These will be combined into an Event and submitted to core-data.
 */
void appsdk_post_readings (devsdk_service_t *svc, const char *profile_name, const char *device_name, const char *resource_name, devsdk_commandresult *values);

#endif //~APPLICATION_SVC_SDK
